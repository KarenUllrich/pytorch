...
#define THC_GENERIC_FILE "generic/SpatialSliceExtractorTrilinear.cu"
#else
...
static inline void THNN_(SpatialSliceExtractorTrilinear_shapeCheck)(
  ...
  int64_t idepth    = THCTensor_(size)(state, input, 4);
  ...
  THCUNN_check_dim_size(state, grid, 4, 3, 3);
...
TH_API void THNN_(SpatialSliceExtractorTrilinear_updateOutput)(
  ...
  THNN_(SpatialSliceExtractorTrilinear_shapeCheck)(state, input, grid, NULL);
  ...
  int64_t ID = THCTensor_(size)(state, input, 4);
  ...
  SpatialSliceExtractorTrilinear_updateOutput_kernel
...
TH_API void THNN_(SpatialSliceExtractorTrilinear_updateGradInput)(
  ...
  THNN_(SpatialSliceExtractorTrilinear_shapeCheck)(state, input, grid, gradOutput);
  int64_t N = THCTensor_(size)(state, input, 0);
  int64_t C = THCTensor_(size)(state, input, 1);
  int64_t IH = THCTensor_(size)(state, input, 2);
  int64_t IW = THCTensor_(size)(state, input, 3);
  ...
  THCTensor_(resize5d)(state, gradInput, N, C, IH, IW, ID);
  THCTensor_(resize4d)(state, gradGrid, N, H, W, 3);
  ...
  int count = static_cast<int>(N*H*W);
  SpatialSliceExtractorTrilinear_updateGradInput_kernel
    <<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state)>>>(
